...
NVIDIA's intended relative-pose default gains of `10.0` translation and `10.0`
rotation. S2 keeps the upstream filter/rebase implementation and records those
two gains in its concrete processor configuration.
...
poll anchor hotkeys. S2 carries squeeze/trigger through its required action and
therefore suppresses that redundant convenience source while inheriting every
...
- Trigger above `0.5` closes the corresponding gripper. The binary Gate C
  aperture targets are `0.0 m` closed and `0.1 m` open; reset uses `0.05 m`.
- Missing, unavailable, invalid, or non-finite tracking emits zero motion and
  marks that arm for rebase. The first recovered valid delta is discarded.
