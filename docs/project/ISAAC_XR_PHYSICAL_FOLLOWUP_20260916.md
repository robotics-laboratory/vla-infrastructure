...
| Wrist camera pose | User first observed a downward, inverted view; after the first optical correction, physical evidence showed that the camera origin was still under the gripper and the output needed a 180° roll. | Corrected through user-selected render F: mount at parent `(-0.060, 0, +0.010)`, roll 180°, and pitch 3° toward the gripper. | Gripper extended; confirm the F framing and orientation in Quest. |
...
The mount-side probe retained orientation `(0.70710678, 0, 0.70710678, 0)` in XYZW while locating the correct physical side. At the demo home pose, gripper-parent +X has world Z component `−0.819`, so the previous `(+0.06, 0, 0)` offset put the camera below the gripper. Moving to parent −X placed it above the gripper.

The next archived real-Kit sweep applied the requested output roll, small lift, pitch, and forward offsets. The user selected variant F: position `(-0.060, 0, +0.010)` m and orientation `(0, -0.68835458, 0, 0.72537437)` in XYZW. This rolls the image 180°, points camera +X mostly along the gripper's +Z approach axis with a 3° component toward parent +X, and maps image-up mostly to parent −X. The 1 cm approach-axis translation reduces the claw obstruction compared with the unshifted variant. The non-XR S1 camera contract remains unchanged. Physical Quest confirmation remains required.
