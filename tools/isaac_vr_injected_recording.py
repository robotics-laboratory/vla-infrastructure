...
def _command(grippers: tuple[float, float], *, left_clutch: bool = False) -> BimanualTeleopCommand:
    arms = tuple(
        ArmTeleopCommand(
            delta_pose=np.zeros(6, dtype=np.float64),
            gripper_aperture_m=gripper,
            motion_active=not (left_clutch and index == 0),
            tracking_valid=True,
            clutch_active=left_clutch and index == 0,
            rebased=False,
            sensitivity_mode="normal",
            translation_scale=1.0,
            rotation_scale=1.0,
            transition="clutch_held" if left_clutch and index == 0 else "motion",
        )
