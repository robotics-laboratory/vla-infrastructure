    ...
    for p in sorted(spec_paths):
        ...
        if p.name not in {"README.md", "NORMATIVE_MODEL.md", "MIGRATION_V4_3_V5_1_TO_V5_2.md"}:
            if "DECIDE/PIN" in text or "MIGRATE/PRESERVE" in text:
                errors.append(f"{p.name}: legacy magic placeholder text forbidden")

        for st in re.findall(r"\bstatus\.[A-Za-z0-9_]+", text):
            errors.append(f"{p.name}: stale/unknown status reference {st}")
        for x in GATE_RE.findall(text):
            if x not in gates:
                errors.append(f"{p.name}: unknown gate {x}")
        for x in PROFILE_RE.findall(text):
            if x not in profiles:
                errors.append(f"{p.name}: unknown profile {x}")
        for x in SOURCE_RE.findall(text):
            if x not in sources:
                errors.append(f"{p.name}: unknown source {x}")
    if errors:
